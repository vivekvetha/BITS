"""
Shared US population preprocessing, model training, and MLflow logging.
Used by MLOps-MLFlow/main.py and Prefect flows/workflow_us_population.py.
"""
...
def default_csv_path() -> str:
    """Prefer Prefect/data copy; fall back to API_Assignment_Code root CSV."""
    root = _project_root()
    preferred = root / "Prefect" / "data" / "us_pop_processed_value.csv"
    fallback = root / "us_pop_processed_value.csv"
...
def configure_mlflow(
    tracking_uri: str | None = None, experiment: str = "us_pop_income"
):
...
def run_mlflow_training_from_xy(X, y, *, test_size=0.3, random_state=42):
    """Train/evaluate three models and log each to MLflow (expects configured tracking)."""
...
def run_mlflow_training_pipeline(csv_path: str | None = None):
    """Load CSV, preprocess, configure MLflow, train three models (CLI / main entry)."""
